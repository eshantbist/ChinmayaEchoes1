...
class QuotesTweetList extends Component{
      ...
      if(tweetsAvailable===false){
        ...
        fetch(url)
           .then(response => {
             response.json()
            .then(json => {
               json.map(category=>this.props.quotesFilter(category.id))
             })
           })
           .catch();
      }
    ...
    render() {
      ...
      const { QuotesTweetsReducer: {
        tweets,tweetsAvailable,imageModalVisibility,image,readMoreModalVisibility,tweet,emptyQuotesData
      }} = this.props;
        ...
        return(
          <View style={styles.scrollContainer}>
              <FlatList
                ...
                renderItem={({item}) => <TweetItem onImagePress={()=>this.toggleModal(item.id)} onPress={()=>this.goToDetail(item.id)} tweet={item}/>}
                />
                {
...
const mapDispatchToProps=(dispatch)=>{
  return bindActionCreators({imageModal,closeReadMoreModal,showReadMoreModal,closeImageModal,logOut,quotesFilter,tweetDetail},dispatch);
}
